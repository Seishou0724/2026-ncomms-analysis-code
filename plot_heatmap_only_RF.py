...
try:
    ...
    X_test = joblib.load(os.path.join(output_dir, f"PVTHE_X_test_rf_kfold_{region}_{year_range}.pkl"))
    combo_test = joblib.load(os.path.join(output_dir, f"PVTHE_combo_test_rf_kfold_{region}_{year_range}.pkl"))
    
    model = joblib.load(os.path.join(output_dir, f"PVTHE_rf_model_kfold_{region}_{year_range}.pkl"))
    feature_scaler = joblib.load(os.path.join(output_dir, f"PVTHE_rf_feature_scaler_kfold_{region}_{year_range}.pkl"))
    target_scaler = joblib.load(os.path.join(output_dir, f"PVTHE_rf_target_scaler_kfold_{region}_{year_range}.pkl"))
...
ax1 = sns.heatmap(ri_index_matrix[::-1], 
                  ...
                  linewidths=0.5, 
                  linecolor='black',
                  cbar_kws={'label': f"Assessed IR Index"}, 
                  annot_kws={"size": 11},
                  vmin=0, vmax=10)
...
plt.title(f"Assessed IR Index by RF - {region} ({year_range})")
plt.xlabel("Time Interval (h)")
plt.ylabel("Wind Speed Increment Threshold (kt)")
heatmap_path1 = os.path.join(output_dir, f"RI_Index_RF_heatmap_accessed_only_{year_range}_{region}.png")
...
for spine in ax2.spines.values():
    ...
    spine.set_color('black')
    spine.set_linewidth(2)
plt.title(f"Assessed IR Index by RF with Difference & MSE - {region} ({year_range})")
plt.xlabel("Time Interval (h)")
plt.ylabel("Wind Speed Increment Threshold (kt)")
