...
train_y_files = {
    "random_forest": os.path.join(train_pkl_dir, "PVTHE_y_test_rf_kfold_upper_inner_2013-2022.pkl"),
    "svr": os.path.join(train_pkl_dir, "PVTHE_y_test_svr_kfold_upper_inner_2013-2022.pkl"),
    "ann": os.path.join(train_pkl_dir, "PVTHE_y_test_ann_kfold_upper_inner_2013-2022.pkl")
}
...
try:
    ...
    rf_y_train = joblib.load(train_y_files["random_forest"])
...
try:
    ...
    svr_y_train = joblib.load(train_y_files["svr"])
...
try:
    ...
    ann_y_train = joblib.load(train_y_files["ann"])
...
def main():
    ...
    def process_model(model_name, y_test_pred, target_scaler, delta_winds_list):
        ...
        bin_labels = [f"Bin {k+1} ({bin_centers[k]:.2f})" for k in range(10)]
        num_bins = 10
        ...
        plt.ylabel("RI Index Bin", fontsize=14)
        ...
        if not df.empty:
            ...
            levels = np.linspace(0.5, 1.0, 6) # 50%, 60%, 70%, 80%, 90%, 100%
            linewidths = [0.5, 1.0, 1.5, 2.0, 2.5, 3.0] # 從細到粗，對應低到高水平
            ...
            plt.yticks(np.arange(min(wind_thresholds), max(wind_thresholds) + 25, 25)) # 每 25kt 一個刻度
            plt.ylim(min(wind_thresholds) - 5, max(wind_thresholds) + 5) # 設置 Y 軸範圍
            # 添加 bin 圖例
            bin_legend_elements = [Line2D([0], [0], color=colors[i], lw=2, label=bin_labels[i]) for i in range(num_bins)]
            bin_legend = plt.legend(handles=bin_legend_elements, title='Bins', loc='upper right', fontsize=14, title_fontsize=16)
            ...
            plt.title(f"2D KDE of Wind Change by Time Interval and RI Index Bin ({model_name}, {test_year_range})", fontsize=16)
            plt.tick_params(axis='both', labelsize=12)
    ...
    if rf_model:
        try:
            ...
            rf_mean_test, rf_y_test_pred = process_model("Random Forest", y_test_pred_rf, rf_target_scaler, delta_winds_list)
        except Exception as e:
            logging.error(f"Random Forest 預測錯誤: {e}")
    # 預測並計算偏差 (SVR)
    svr_mean_test = None
    svr_y_test_pred = None
    if svr_model:
        try:
            ...
            svr_mean_test, svr_y_test_pred = process_model("SVR", y_test_pred_svr, svr_target_scaler, delta_winds_list)
        except Exception as e:
            logging.error(f"SVR 預測錯誤: {e}")
    # 預測並計算偏差 (ANN)
    ann_mean_test = None
    ann_y_test_pred = None
    if ann_model:
        try:
            ...
            ann_mean_test, ann_y_test_pred = process_model("ANN", y_test_pred_ann, ann_target_scaler, delta_winds_list)
        except Exception as e:
            logging.error(f"ANN 預測錯誤: {e}")
    # 計算模型與驗證資料平均 RI index 的差異及顯著性分析
    if rf_model and rf_y_train is not None and rf_y_test_pred is not None:
        rf_train_ri_scaled = rf_target_scaler.transform(rf_y_train.reshape(-1, 1)).ravel()
        rf_mean_train = np.mean(rf_train_ri_scaled)
        rf_diff = rf_mean_test - rf_mean_train
        t_stat_rf, p_value_rf = ttest_ind(rf_y_test_pred, rf_train_ri_scaled)
        logging.info(f"Random Forest Mean RI Difference (Test - Train): {rf_diff:.2f}")
        logging.info(f"Random Forest Difference Significance: t-stat={t_stat_rf:.2f}, p-value={p_value_rf:.4f}")
        print(f"Random Forest Mean RI Difference (Test - Train): {rf_diff:.2f}")
        print(f"Random Forest Difference Significance: t-stat={t_stat_rf:.2f}, p-value={p_value_rf:.4f}")
    if svr_model and svr_y_train is not None and svr_y_test_pred is not None:
        svr_train_ri_scaled = svr_target_scaler.transform(svr_y_train.reshape(-1, 1)).ravel()
        svr_mean_train = np.mean(svr_train_ri_scaled)
        svr_diff = svr_mean_test - svr_mean_train
        t_stat_svr, p_value_svr = ttest_ind(svr_y_test_pred, svr_train_ri_scaled)
        logging.info(f"SVR Mean RI Difference (Test - Train): {svr_diff:.2f}")
        logging.info(f"SVR Difference Significance: t-stat={t_stat_svr:.2f}, p-value={p_value_svr:.4f}")
        print(f"SVR Mean RI Difference (Test - Train): {svr_diff:.2f}")
        print(f"SVR Difference Significance: t-stat={t_stat_svr:.2f}, p-value={p_value_svr:.4f}")
    if ann_model and ann_y_train is not None and ann_y_test_pred is not None:
        ann_train_ri_scaled = ann_target_scaler.transform(ann_y_train.reshape(-1, 1)).ravel()
        ann_mean_train = np.mean(ann_train_ri_scaled)
        ann_diff = ann_mean_test - ann_mean_train
        t_stat_ann, p_value_ann = ttest_ind(ann_y_test_pred, ann_train_ri_scaled)
        logging.info(f"ANN Mean RI Difference (Test - Train): {ann_diff:.2f}")
        logging.info(f"ANN Difference Significance: t-stat={t_stat_ann:.2f}, p-value={p_value_ann:.4f}")
        print(f"ANN Mean RI Difference (Test - Train): {ann_diff:.2f}")
        print(f"ANN Difference Significance: t-stat={t_stat_ann:.2f}, p-value={p_value_ann:.4f}")
