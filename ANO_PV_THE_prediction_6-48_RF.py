...
def train_model(sid_data, output_dir, year_range, region, delta=0):
    ...
    for lead in time_intervals:
        ...
        for train_idx, val_idx in kf.split(X_train_lead):
            ...
            rf = RandomForestRegressor(n_estimators=40, max_depth=4, min_samples_leaf=10, random_state=42, n_jobs=1) # 步驟2: 減過擬合，調參
            rf.fit(X_fold_train_pca, y_fold_train)
            y_pred_val = rf.predict(X_fold_val_pca)
        ...
        rf1 = RandomForestRegressor(n_estimators=40, max_depth=4, min_samples_leaf=10, random_state=42, n_jobs=1)
        rf2 = RandomForestRegressor(n_estimators=50, max_depth=4, min_samples_leaf=8, random_state=123, n_jobs=1)
        rf3 = RandomForestRegressor(n_estimators=30, max_depth=3, min_samples_leaf=15, random_state=456, n_jobs=1)
        rf4 = RandomForestRegressor(n_estimators=40, max_depth=3, min_samples_leaf=10, random_state=789, n_jobs=1)
        ensemble = VotingRegressor([('rf1', rf1), ('rf2', rf2), ('rf3', rf3), ('rf4', rf4)])
        ensemble.fit(X_train_lead_pca, y_train_clipped) # Use clipped for training
        models[lead] = ensemble
        # Save per lead model
        model_path = os.path.join(output_dir, f"rf_model_{lead}h_{region}_{year_range}_delta{delta}.pkl")
        joblib.dump(ensemble, model_path)
        # Save per lead scaler
        scaler_path = os.path.join(output_dir, f"rf_scaler_{lead}h_{region}_{year_range}_delta{delta}.pkl")
        joblib.dump(scaler_lead, scaler_path)
        # Save per lead PCA
        pca_path = os.path.join(output_dir, f"rf_pca_{lead}h_{region}_{year_range}_delta{delta}.pkl")
    ...
    for lead in models:
        ...
        rf1 = ensemble.named_estimators_['rf1']
        top_features = np.argsort(rf1.feature_importances_)[-5:]
        print(f"{lead}h delta{delta} rf1 top 5 features indices: {top_features} (importance: {rf1.feature_importances_[top_features]})")
...
def evaluate_test(test_samples, models, scalers, pcas, output_dir, year_range, region, delta=0):
    ...
    if plot_samples:
        ...
        ax.set_title(f'Forecast Bias of RF for All Leads per Sample - {region} ({year_range}) delta{delta}')
        ax.set_xlim(0, 50)
        ax.set_xticks(time_intervals)
        ax.set_yticks(np.arange(-100, 101, 10))
        ax.grid(True)
        ax.legend(loc='upper right')
        error_plot_path = os.path.join(output_dir, f"rf_bias_by_sample_all_leads_fixed_{region}_{year_range}_delta{delta}.png")
    ...
    for lead in time_intervals:
        ...
        plot_path = os.path.join(output_dir, f"rf_test_scatter_{lead}h_{region}_{year_range}_delta{delta}.png")
...
def main():
    ...
    for year_range in year_ranges:
        ...
        log_file = os.path.join(output_dir, f"rf_tc_intensity_{year_range}.log")
        ...
        if biases_dict:
            ...
            ax.set_yticks(np.arange(-100, 101, 10))
            ax.grid(True)
            combined_plot_path = os.path.join(output_dir, f"rf_bias_combined_all_deltas_{region}_{year_range}.png")
