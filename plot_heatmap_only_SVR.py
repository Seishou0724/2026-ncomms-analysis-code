...
# ====================== 設定 ======================
year_range = "2013-2022"   # ←←← 改成你要畫的年代 (1959-1980 / 1981-2012 / 2013-2022)
region = "upper_inner"
...
try:
    ...
    X_test = joblib.load(os.path.join(output_dir, f"PVTHE_X_test_svr_kfold_{region}_{year_range}.pkl"))
    combo_test = joblib.load(os.path.join(output_dir, f"PVTHE_combo_test_svr_kfold_{region}_{year_range}.pkl"))
    
    model = joblib.load(os.path.join(output_dir, f"PVTHE_svr_model_kfold_{region}_{year_range}.pkl"))
    feature_scaler = joblib.load(os.path.join(output_dir, f"PVTHE_svr_feature_scaler_kfold_{region}_{year_range}.pkl"))
    target_scaler = joblib.load(os.path.join(output_dir, f"PVTHE_svr_target_scaler_kfold_{region}_{year_range}.pkl"))
...
ax1 = sns.heatmap(ri_index_matrix[::-1], 
                  ...
                  linewidths=0.5, 
                  linecolor='black',
                  cbar_kws={'label': f"Assessed IR Index - {region}"}, 
                  annot_kws={"size": 11},
                  vmin=0, vmax=10)
...
plt.title(f"Assessed IR Index by SVR - {region} ({year_range})")
plt.xlabel("Time Interval (h)")
plt.ylabel("Wind Speed Increment Threshold (kt)")
heatmap_path1 = os.path.join(output_dir, f"RI_Index_svr_heatmap_accessed_only_{year_range}_{region}.png")
plt.savefig(heatmap_path1, bbox_inches='tight', dpi=300)
plt.close()
logging.info(f"主要熱圖 (只顯示 Accessed IR Index) 已儲存至 {heatmap_path1}")
...
ax2 = sns.heatmap(diff_matrix[::-1], 
                  ...
                  linewidths=0.5, 
                  linecolor='black',
                  cbar_kws={'label': f"Difference (Assessed - Target) IR Index"}, 
                  annot_kws={"size": 11},
                  vmin=-10, vmax=10)
for spine in ax2.spines.values():
    ...
    spine.set_color('black')
    spine.set_linewidth(2)
plt.title(f"Assessed IR Index by SVR with Difference & MSE - {region} ({year_range})")
plt.xlabel("Time Interval (h)")
plt.ylabel("Wind Speed Increment Threshold (kt)")
heatmap_path2 = os.path.join(output_dir, f"RI_Index_svr_heatmap_with_diff_mse_{year_range}_{region}.png")
