...
from tensorflow.keras.layers import Dense, Input  # 新增 Input
...
def train_model(sid_data, output_dir, year_range, region, delta=0):
    ...
    for lead in time_intervals:
        ...
        for train_idx, val_idx in kf.split(X_train_lead):
            ...
            ann = Sequential()
            ann.add(Input(shape=(X_fold_train_pca.shape[1],)))  # 修正：使用 Input 層
            ann.add(Dense(64, activation='relu'))
            ann.add(Dense(64, activation='relu'))
            ann.add(Dense(1, activation='linear'))
            ann.compile(optimizer=Adam(learning_rate=0.001), loss='mse')
            ann.fit(X_fold_train_pca, y_fold_train, epochs=50, batch_size=32, verbose=0)
            y_pred_val = ann.predict(X_fold_val_pca, verbose=0).flatten()  # 加 verbose=0 避免進度條
        ...
        ann = Sequential()
        ann.add(Input(shape=(X_train_lead_pca.shape[1],)))  # 修正：使用 Input 層
        ann.add(Dense(64, activation='relu'))
        ann.add(Dense(64, activation='relu'))
        ann.add(Dense(1, activation='linear'))
        ann.compile(optimizer=Adam(learning_rate=0.001), loss='mse')
        ann.fit(X_train_lead_pca, y_train_clipped, epochs=50, batch_size=32, verbose=0) # Use clipped for training
        models[lead] = ann
        # Save per lead model
        model_path = os.path.join(output_dir, f"ann_model_{lead}h_{region}_{year_range}_delta{delta}.h5")
        ann.save(model_path)
        # Save per lead scaler
        scaler_path = os.path.join(output_dir, f"ann_scaler_{lead}h_{region}_{year_range}_delta{delta}.pkl")
        joblib.dump(scaler_lead, scaler_path)
        # Save per lead PCA
        pca_path = os.path.join(output_dir, f"ann_pca_{lead}h_{region}_{year_range}_delta{delta}.pkl")
...
def evaluate_test(test_samples, models, scalers, pcas, output_dir, year_range, region, delta=0):
    ...
    for s in test_samples:
        ...
        if len(available_leads) > 0:
            ...
            for lead in available_leads:
                ...
                predicted = models[lead].predict(features_pca, verbose=0)[0][0]  # 加 verbose=0 避免進度條
    ...
    if plot_samples:
        ...
        ax.set_title(f'Forecast Bias of ANN for All Leads per Sample - {region} ({year_range}) delta{delta}')
        ax.set_xlim(0, 50)
        ax.set_xticks(time_intervals)
        ax.set_yticks(np.arange(-100, 101, 10))
        ax.grid(True)
        ax.legend(loc='upper right')
        error_plot_path = os.path.join(output_dir, f"ann_bias_by_sample_all_leads_fixed_{region}_{year_range}_delta{delta}.png")
    ...
    for lead in time_intervals:
        ...
        y_pred_lead = models[lead].predict(features_test_pca, verbose=0).flatten()  # 加 verbose=0 避免進度條
        ...
        plot_path = os.path.join(output_dir, f"ann_test_scatter_{lead}h_{region}_{year_range}_delta{delta}.png")
...
def main():
    ...
    for year_range in year_ranges:
        ...
        log_file = os.path.join(output_dir, f"ann_tc_intensity_{year_range}.log")
        ...
        if biases_dict:
            ...
            ax.set_yticks(np.arange(-100, 101, 10))
            ax.grid(True)
            combined_plot_path = os.path.join(output_dir, f"ann_bias_combined_all_deltas_{region}_{year_range}.png")
