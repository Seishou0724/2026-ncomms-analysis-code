...
# ====================== 主函數（已轉成SVR） ======================
def main():
    logging.info("程式開始執行 - SVR - 每個 (wind,time) 組合獨立 80/20 split")
    ...
    if len(X_train) < 5:
        ...
        svr_final = SVR(kernel='rbf', C=1.0, epsilon=0.1, gamma='scale')
    ...
    # ====================== 儲存 ======================
    test_prefix = "PVTHE"
    joblib.dump(X_test, os.path.join(output_dir, f"{test_prefix}_X_test_svr_onset_{region}_{year_range}.pkl"))
    joblib.dump(y_test, os.path.join(output_dir, f"{test_prefix}_y_test_svr_onset_{region}_{year_range}.pkl"))
    joblib.dump(combo_test, os.path.join(output_dir, f"{test_prefix}_combo_test_svr_onset_{region}_{year_range}.pkl"))
    joblib.dump(delta_test, os.path.join(output_dir, f"{test_prefix}_delta_winds_svr_onset_{region}_{year_range}.pkl"))
    model_path = os.path.join(output_dir, f"PVTHE_svr_model_onset_{region}_{year_range}.pkl")
    scaler_path = os.path.join(output_dir, f"PVTHE_svr_feature_scaler_onset_{region}_{year_range}.pkl")
    scaler_y_path = os.path.join(output_dir, f"PVTHE_svr_target_scaler_onset_{region}_{year_range}.pkl")
    joblib.dump(svr_final, model_path)
    joblib.dump(scaler, scaler_path)
    joblib.dump(scaler_y, scaler_y_path)
    logging.info(f"SVR 模型已儲存至 {model_path}")
    logging.info("程式執行完畢 - SVR 版本")
