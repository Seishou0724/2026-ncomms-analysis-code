...
# 定義年份範圍
year_range = "1959-1980"

# 設置輸出目錄
output_dir = f"/Dellwork6/cwusei/RI/ALL_IBTrACS/Program/PYTHON/RIIndex/{year_range}-Samplesize"
...
logging.info("腳本開始執行 - 計算 RI 閾值樣本數熱圖")
...
for i, wind in enumerate(wind_thresholds):
    for j, time in enumerate(time_intervals):
        ...
        if os.path.exists(pv_path_ri):
            ...
            sample_count = len(pv_files)
            samples_matrix[i, j] = sample_count
            logging.info(f"Wind {wind} kt, Time {time} h: {sample_count} 個樣本")
        else:
...
ax = sns.heatmap(samples_matrix[::-1], xticklabels=time_intervals, yticklabels=wind_thresholds[::-1],
                 cmap="YlOrRd", annot=annot_matrix[::-1], fmt="", linewidths=0.5, linecolor='black',
                 cbar_kws={'label': "Sample Size per RI Threshold"}, annot_kws={"size": 8})
...
plt.title(f"RI Threshold Sample Size Heatmap - ({year_range})")
plt.xlabel("Time Interval (h)")
plt.ylabel("Wind Speed Threshold (kt)")

heatmap_path = os.path.join(output_dir, f"RI_SampleSize_Heatmap_{year_range}.png")
