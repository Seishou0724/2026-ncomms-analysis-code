...
log_file_path = os.path.join(output_dir, f"ANO_RII_svr_continue_{year_range}.log")
logging.basicConfig(
    level=logging.INFO,
...
def main():
    ...
    X_test_path = os.path.join(output_dir, f"PVTHE_X_test_svr_kfold_{region}_{year_range}.pkl")
    y_test_path = os.path.join(output_dir, f"PVTHE_y_test_svr_kfold_{region}_{year_range}.pkl")
    combo_test_path = os.path.join(output_dir, f"PVTHE_combo_test_svr_kfold_{region}_{year_range}.pkl")
    delta_winds_path = os.path.join(output_dir, f"PVTHE_delta_winds_svr_kfold_{region}_{year_range}.pkl")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X_train_scaled, stratify_train_labels)):
        ...
        y_val_pred = best_svr.predict(X_val_fold)
    ...
    axes[2].legend()
    plt.tight_layout()
    cv_plot_path = os.path.join(output_dir, f"SVR_Kfold_CV_results_{region}_{year_range}.png")
    ...
    svr_final = SVR(**best_params_dict)
    svr_final.fit(X_train_scaled, y_train_scaled)
    model_path = os.path.join(output_dir, f"PVTHE_svr_model_kfold_{region}_{year_range}.pkl")
    scaler_path = os.path.join(output_dir, f"PVTHE_svr_feature_scaler_kfold_{region}_{year_range}.pkl")
    scaler_y_path = os.path.join(output_dir, f"PVTHE_svr_target_scaler_kfold_{region}_{year_range}.pkl")
    joblib.dump(svr_final, model_path)
    joblib.dump(scaler, scaler_path)
    joblib.dump(scaler_y, scaler_y_path)
    logging.info(f"已儲存 SVR 模型至 {model_path}")
    ...
    plt.title(f"Assessed RI Index by SVR (MSE) - {region} ({year_range})")
    plt.xlabel("Time Interval (h)")
    plt.ylabel("Wind Speed Increment Threshold (kt)")
    heatmap_path = os.path.join(output_dir, f"PV_THE_ANO_ri_index_heatmap_svr_kfold_{year_range}_{region}.png")
