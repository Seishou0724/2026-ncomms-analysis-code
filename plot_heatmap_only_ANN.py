...
try:
    ...
    X_test = joblib.load(os.path.join(output_dir, f"PVTHE_X_test_ann_kfold_{region}_{year_range}.pkl"))
    combo_test = joblib.load(os.path.join(output_dir, f"PVTHE_combo_test_ann_kfold_{region}_{year_range}.pkl"))
    
    model = joblib.load(os.path.join(output_dir, f"PVTHE_ann_model_kfold_{region}_{year_range}.pkl"))
    feature_scaler = joblib.load(os.path.join(output_dir, f"PVTHE_ann_feature_scaler_kfold_{region}_{year_range}.pkl"))
    target_scaler = joblib.load(os.path.join(output_dir, f"PVTHE_ann_target_scaler_kfold_{region}_{year_range}.pkl"))
...
plt.title(f"Assessed IR Index by ANN - {region} ({year_range})")
plt.xlabel("Time Interval (h)")
plt.ylabel("Wind Speed Increment Threshold (kt)")
heatmap_path1 = os.path.join(output_dir, f"RI_Index_ann_heatmap_accessed_only_{year_range}_{region}.png")
...
for spine in ax2.spines.values():
    ...
    spine.set_color('black')
    spine.set_linewidth(2)
plt.title(f"Assessed IR Index by ANN with Difference & MSE - {region} ({year_range})")
plt.xlabel("Time Interval (h)")
plt.ylabel("Wind Speed Increment Threshold (kt)")
heatmap_path2 = os.path.join(output_dir, f"RI_Index_ann_heatmap_with_diff_mse_{year_range}_{region}.png")
