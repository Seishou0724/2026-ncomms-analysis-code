...
def main():
    ...
    X_test_path = os.path.join(output_dir, f"PVTHE_X_test_ann_kfold_{region}_{year_range}.pkl")
    y_test_path = os.path.join(output_dir, f"PVTHE_y_test_ann_kfold_{region}_{year_range}.pkl")
    combo_test_path = os.path.join(output_dir, f"PVTHE_combo_test_ann_kfold_{region}_{year_range}.pkl")
    delta_winds_path = os.path.join(output_dir, f"PVTHE_delta_winds_ann_kfold_{region}_{year_range}.pkl")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X_train_scaled, stratify_train_labels)):
        ...
        grid_search = GridSearchCV(ann, param_grid, cv=3, scoring='neg_mean_squared_error', n_jobs=-1)
    ...
    axes[2].legend()
    plt.tight_layout()
    cv_plot_path = os.path.join(output_dir, f"ANN_Kfold_CV_results_{region}_{year_range}.png")
    ...
    ann_final = MLPRegressor(**best_params_dict, random_state=42, max_iter=500)
    ann_final.fit(X_train_scaled, y_train_scaled)
    model_path = os.path.join(output_dir, f"PVTHE_ann_model_kfold_{region}_{year_range}.pkl")
    scaler_path = os.path.join(output_dir, f"PVTHE_ann_feature_scaler_kfold_{region}_{year_range}.pkl")
    scaler_y_path = os.path.join(output_dir, f"PVTHE_ann_target_scaler_kfold_{region}_{year_range}.pkl")
    joblib.dump(ann_final, model_path)
    joblib.dump(scaler, scaler_path)
    joblib.dump(scaler_y, scaler_y_path)
    logging.info(f"已儲存 ANN 模型至 {model_path}")
    ...
    plt.title(f"Assessed RI Index by ANN (Diff / MSE) - {region} ({year_range})")
    plt.xlabel("Time Interval (h)")
    plt.ylabel("Wind Speed Increment Threshold (kt)")
    heatmap_path = os.path.join(output_dir, f"PV_THE_ANO_ri_index_heatmap_ann_kfold_{year_range}_{region}.png")
